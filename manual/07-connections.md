...
VS Code 세션에 Computer Use 도구가 실제 노출되는지 확인합니다.
노출되지 않으면 같은 AI Desk를 공식 데스크톱 앱에서 열어 화면 조작 업무를 진행합니다.
IDE에 없는 기능을 설치 완료라고 표시하거나 비공식 MCP로 조용히 대체하지 않습니다.
사용 중인 앱·계정·지역에 기능이 없으면 setup-status.md에 미제공으로 기록합니다.
시험은 소유자가 허용한 연습 문서를 읽는 것으로 하며 민감한 앱은 열지 않습니다.
