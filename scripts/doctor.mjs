...
try {
  ...
  for(const f of ['.obsidian/app.json','.obsidian/appearance.json','.obsidian/core-plugins.json','.vscode/settings.json','.vscode/extensions.json'])check(`JSON ${f}`,()=>!!JSON.parse(fs.readFileSync(path.join(opts.target,f),'utf8')));
  ...
  check('Obsidian 표준 링크 설정',()=>{const a=JSON.parse(fs.readFileSync(path.join(opts.target,'.obsidian/app.json'),'utf8'));return a.useMarkdownLinks===true&&a.newLinkFormat==='relative';});
